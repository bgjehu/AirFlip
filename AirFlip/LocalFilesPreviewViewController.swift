...
class LocalFilesPreviewViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, SKCacheManagerDelegate {
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        performSegueWithIdentifier("presentSlidesSegue", sender: dataSource![indexPath.row])
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "presentSlidesSegue" {
            (segue.destinationViewController as! SlidesViewController).slidesInfo = sender as? SKInfo
        }
    }
}
